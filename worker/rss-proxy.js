...
//                     "short": true|false } ] }
//
// 2026-08-29: 영상마다 **쇼츠인지 롱폼인지**를 붙인다(`short`). 알아내지 못하면
//  **필드를 안 붙인다** — 아래 markShorts 참고. 기존 필드는 그대로 두고 추가만 하는
//  규약이라, 옛 화면이 새 응답을 받아도 안 깨진다.
//
...
async function fetchFeed(channelId, limit) {
  ...
  // 쇼츠 판정은 **영상이 있을 때만** 한다 (아래 0개 분기보다 먼저 걸릴 이유가 없다).
...
// ────────────────────────── 쇼츠인가 롱폼인가 (2026-08-29) ──────────────────────────
// ⚠️ **RSS에는 이 정보가 아예 없다.** 길이도 없고 쇼츠 표시도 없다 — 쇼츠도 링크가
//  `/watch?v=`로 온다(2026-08-29 실제 피드로 확인). 그래서 **다른 문에 한 번 더 묻는다.**
//  confirmGone과 같은 수법이다: 한 문이 답을 모르면 별개로 서비스되는 다른 문에 묻는다.
//
// ⭐ 판별법 — `youtube.com/shorts/<id>`를 **따라가지 않고** 열어본다.
//   200 → 쇼츠 (그 주소가 진짜 쇼츠 플레이어다)
//   303 → 롱폼 (`/watch?v=…`로 되돌려 보낸다)
//
// ⭐ **왜 «길이 ≤ 60초» 규칙을 안 쓰나** (이 판별법을 고른 진짜 이유):
//  실측(2026-08-29)에서 200으로 판정된 것들이 29초·**81초**·**107초**였고 303은 494초·956초였다.
//  유튜브가 쇼츠 상한을 3분으로 늘렸고 세로 여부·업로드 방식도 조건에 든다 —
//  **길이는 우리가 만든 짐작이고, 이 리다이렉트는 유튜브 자신의 판정이다.**
//  (길이는 Data API `videos.list`로 받을 수 있지만 로그인이 필요하다. 그러면 로그인 안 한
//   사람에겐 배지가 없는 반쪽이 된다 — 피드는 로그인 없이도 되는 화면이라 그쪽은 안 맞는다.)
//
// ⚠️ **HEAD로 부른다.** GET이면 쇼츠일 때 페이지 본문(1MB대)이 통째로 딸려온다 —
//  상태코드 하나 보자고 그걸 받을 이유가 없다. (실측: HEAD는 본문 0바이트)
//
// ⭐ **모르면 필드를 안 붙인다.** 404·5xx·429·네트워크 실패는 «롱폼»이 아니라 «모른다»다.
//  2026-08-21에 배운 것(404는 증거이지 증명이 아니다)을 여기서도 지킨다. 화면은 필드가
//  없으면 아무 배지도 안 그린다 — 옛 Worker가 보낸 응답과 똑같이 취급된다.
//
// ⬜ **캐시는 아직 안 붙인다.** 쇼츠 여부는 바뀌지 않는 사실이라 캐시해도 되는 몇 안 되는
//  값인데(08-21에 걷어낸 것은 «틀릴 수 있는 판정»의 캐시였다), **workers.dev 주소에서는
//  Cache API가 동작하지 않고** `cf.cacheTtl`은 GET에만 걸린다. 🚦 신호등: 피드가 눈에 띄게
//  느려지거나 유튜브가 스로틀링을 시작할 때 → 커스텀 도메인으로 옮기고 Cache API를 쓴다.
    ...
    videos.map(async (v) => {
      ...
      try {
        const res = await fetch(`https://www.youtube.com/shorts/${v.id}`, {
          method: "HEAD",
          redirect: "manual", // ⛔ 따라가면 200으로 보여서 전부 쇼츠가 된다. 여기가 핵심이다
          headers: { "User-Agent": UA },
          // ⚠️ **타임아웃이 없으면 «영상 배지 하나»가 채널 전체 응답을 붙잡는다.**
          //  이 판정은 곁가지이므로 오래 걸리면 그냥 모르는 채로 둔다
          //  (api/feedback.ts가 디스코드 웹훅에 3초를 건 것과 같은 판단).
