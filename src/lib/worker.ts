// worker.ts — 내 Cloudflare Worker를 부르는 유일한 창구.
//
// 지금은 **RSS 채널 목록 전용**이다 (2026-07-31).
//  재생목록·영상 상세는 구글 로그인 뒤 브라우저가 유튜브를 직접 부른다 → youtube.ts
//  RSS만 여기 남은 이유: 유튜브 RSS는 CORS를 허락하지 않아 릴레이가 꼭 필요하고,
//  대신 **로그인 없이도 되고 쿼터를 안 먹는다**. 채널 피드에는 이쪽이 계속 유리하다.
//
// 경계: **여기는 Worker가 무엇을 아는지만 담는다.**
//  사용자에게 보여줄 한국어 문장은 화면이 만든다. 여기서는 판정(err.notFound·state)만 넘긴다.
...
import type { AppError, ChannelFeed } from "./types";
